...
class GraphVisualizer:
    ...
    def draw(
        ...
        """Инкрементальная перерисовка графа."""
        # 1. Подготовка данных и расчет лейаута
        old_coords = dict(self._node_coords)
        coords = self.compute_layout(nodes, edges)
        nodes_by_id = {n.id: n for n in nodes}

        # 2. Обновление узлов (удаление старых, перемещение изменившихся, создание новых)
        self._update_node_actors(nodes_by_id, coords, old_coords)

        # 3. Обновление ребер (полная перерисовка для простоты)
        self._update_edge_actors(edges, coords)

        # 4. Обновление вертикальной оси веток
        self._update_branch_axis(nodes, coords)

        # 5. Управление камерой и окном
        self.plotter.reset_camera()
        self._manage_render_window(show)

    def _update_node_actors(self, nodes_by_id: Dict[str, GraphNode], coords: Dict, old_coords: Dict):
        radii = {"commit": 0.08, "branch": 0.12, "head": 0.16}
        colors = {"commit": "skyblue", "branch": "orange", "head": "red"}

        new_ids = set(nodes_by_id.keys())
        current_ids = set(self._node_actors.keys())

        # Удаляем узлы, которых больше нет или которые передвинулись
        for nid in current_ids:
            if nid not in new_ids or old_coords.get(nid) != coords.get(nid):
                sphere_actor, label_actor = self._node_actors.pop(nid)
                self.plotter.remove_actor(sphere_actor)
                self.plotter.remove_actor(label_actor)

        # Создаем недостающие узлы
        for nid in new_ids:
            if nid not in self._node_actors:
                node = nodes_by_id[nid]
                x, y, z = coords[nid]

                # Сфера
                sphere = pv.Sphere(radius=radii[node.kind], center=(x, y, z))
                opacity = 0.3 if node.kind == "commit" else 1.0
                s_actor = self.plotter.add_mesh(sphere, color=colors[node.kind], opacity=opacity)

                # Текст
                label_pos = (x, y + radii[node.kind] * 2.0, z) if node.kind != "commit" else (x, y, z)
                l_actor = self.plotter.add_point_labels(
                    [label_pos], [node.label], font_size=10, text_color="black", point_size=0
                )
                self._node_actors[nid] = (s_actor, l_actor)

    def _update_edge_actors(self, edges: List[GraphEdge], coords: Dict):
        ...
        for e in edges:
            if e.source in coords and e.target in coords:
                line = pv.Line(coords[e.source], coords[e.target])
                color = "gray" if e.kind == "parent" else "green"
                self._edge_actors[(e.source, e.target, e.kind)] = self.plotter.add_mesh(
                    line, color=color, line_width=2
                )

    def _update_branch_axis(self, nodes: List[GraphNode], coords: Dict):
        """Рисует вертикальную направляющую линию через ветки."""
        if self._branch_axis_actor:
        ...
        if "main" not in branch_ids or "main" not in coords:
            return

        x_m, y_m, _ = coords["main"]
        z_values = [coords[bid][2] for bid in branch_ids if bid in coords]

        if z_values:
            margin = 0.4
            axis_line = pv.Line((x_m, y_m, min(z_values) - margin), (x_m, y_m, max(z_values) + margin))
            self._branch_axis_actor = self.plotter.add_mesh(axis_line, color="black", line_width=1)

    def _manage_render_window(self, show: bool):
